 ...
 *
 * The export writes `conn_country` as a two-letter code. "NG (615,695)" means
 * nothing to most people, so every code is resolved to a readable name before
 * it reaches the screen.
 *
 * Spotify also emits two non-ISO values seen in real exports:
 *   ZZ — unknown / not determined
 *   A1 — anonymous proxy or VPN
 * Both are handled explicitly rather than falling through as raw codes.
 */
...
export const COUNTRY_NAMES = {
  ...
  ML: 'Mali', MM: 'Myanmar', MN: 'Mongolia', MO: 'Macao', MQ: 'Martinique',
  ...
  // Non-ISO values Spotify actually emits.
  ZZ: 'Unknown location',
  A1: 'Anonymous proxy or VPN',
  A2: 'Satellite connection',
};

/** Readable name for a code. Unknown codes come back as the code itself. */
...
/** Regional-indicator flag emoji. Returns '' for the non-ISO pseudo-codes. */
export function countryFlag(code) {
  const key = String(code || '').trim().toUpperCase();
  if (!/^[A-Z]{2}$/.test(key) || key === 'ZZ' || key === 'A1' || key === 'A2') return '';
